...
SSH_GEN_HOST="${SSH_GEN_HOST:-${SSH_GEN_DEFAULTS[host]}}"
SSH_GEN_COMMENT="${SSH_GEN_COMMENT:-$(hostname -f)}"
...
ssh_gen_trap_help() {
  ...
  echo "
    ...
    SSH_GEN_HOSTNAME  - (='${SSH_GEN_DEFAULTS[hostname]}') The actual SSH host. When '%h' (the
   ,                    target hostname) used, must apply SSH_GEN_DEST_DIRNAME
    SSH_GEN_PORT      - (='${SSH_GEN_DEFAULTS[port]}') SSH port
    SSH_GEN_HOST      - (='${SSH_GEN_DEFAULTS[host]}') SSH host match pattern
